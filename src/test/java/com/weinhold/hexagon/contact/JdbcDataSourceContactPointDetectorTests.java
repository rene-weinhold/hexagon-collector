...
class JdbcDataSourceContactPointDetectorTests {
	...
	void reportsDatabaseForDeclaredRepositories() {
		MockEnvironment environment = new MockEnvironment()
				.withProperty("spring.datasource.url", "jdbc:postgresql://db:5432/orders");
	...
	void keepsCredentialsOutOfTheContactPoint() {
		...
		assertThat(contribution.contactPoints().toString()).doesNotContain("s3cr3t").doesNotContain("orders_app")
	...
	void contributesNothingWithoutADatabase() {
		JdbcDataSourceContactPointDetector detector = new JdbcDataSourceContactPointDetector(noDataSource(),
				new MockEnvironment());
