...
class KafkaListenerContactPointDetectorTests {
	...
	void reportsConsumedTopicsAsInboundContactPoints() {
		Contribution contribution = new KafkaListenerContactPointDetector(new MockEnvironment())
				.detect(context(OrderEventsListener.class));
	...
	void resolvesTopicPlaceholders() {
		...
		Contribution contribution = new KafkaListenerContactPointDetector(environment)
				.detect(context(PlaceholderListener.class));

		assertThat(contribution.contactPoints()).extracting(ContactPointInfo::key)
				.containsExactly("kafka:topic/orders.placed");
	}
	...
	void reportsSendToTopicsAsOutboundContactPoints() {
		Contribution contribution = new KafkaListenerContactPointDetector(new MockEnvironment())
				.detect(context(ForwardingListener.class));

		assertThat(contribution.contactPoints())
				.anySatisfy(point -> {
	...
	void doesNotReportSendToWithoutAKafkaListener() {
		Contribution contribution = new KafkaListenerContactPointDetector(new MockEnvironment())
				.detect(context(BareForwarder.class));

		assertThat(contribution.isEmpty()).isTrue();
	}

	private static AdapterContext context(Class<?> type) {
		AdapterInfo base = new AdapterInfo(type.getName(), type.getSimpleName(), Direction.PRIMARY, null,
