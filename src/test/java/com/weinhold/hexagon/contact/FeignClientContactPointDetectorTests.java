...
class FeignClientContactPointDetectorTests {
	...
	void takesTheTargetServiceFromTheAnnotation() {
		Contribution contribution = new FeignClientContactPointDetector(new MockEnvironment(), Map.of())
	...
	void marksAFixedUrlAsDeclaredRatherThanDiscovered() {
		Contribution contribution = new FeignClientContactPointDetector(new MockEnvironment(), Map.of())
				.detect(context(LegacyClient.class));

		assertThat(contribution.contactPoints()).singleElement().satisfies(
				point -> assertThat(point.target().resolution()).isEqualTo(Resolution.ANNOTATION));
	}
	...
	void resolvesPlaceholdersInTheServiceName() {
		...
		Contribution contribution = new FeignClientContactPointDetector(environment, Map.of())
				.detect(context(TemplatedClient.class));

		assertThat(contribution.contactPoints()).singleElement()
				.satisfies(point -> assertThat(point.key()).isEqualTo("http:inventory-service:GET /api/items"));
	}
