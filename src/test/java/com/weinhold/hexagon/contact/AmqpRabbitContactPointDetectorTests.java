...
class AmqpRabbitContactPointDetectorTests {
	...
	void reportsBoundExchangeAsInboundAndSendToAsOutbound() {
		Contribution contribution = new AmqpRabbitContactPointDetector(new MockEnvironment())
				.detect(context(ShipmentListener.class));
	...
	static class ShipmentListener {
		...
		@RabbitListener(bindings = @QueueBinding(value = @Queue("shipment.queue"),
				exchange = @Exchange("shipping"), key = "shipment.dispatched"))
