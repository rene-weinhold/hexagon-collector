...
class HexagonConventionsTests {
	...
	void readsAPublisherAsAnAdapterRatherThanAnEvent() {
		...
		assertThat(classify(OrderEventPublisher.class))
				.isEqualTo(new Classification(Stereotype.ADAPTER, Direction.SECONDARY));
	}
	...
	void classifiesNothingWhenDisabled() {
		assertThat(HexagonConventions.disabled().classify(OrderController.class.getName(),
