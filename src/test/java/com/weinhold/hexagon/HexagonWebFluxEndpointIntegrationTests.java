...
class HexagonWebFluxEndpointIntegrationTests {
	...
	void attachesReactiveRoutesToTheControllerAdapter() {
		AdapterInfo controller = this.endpoint.hexagon().adapters().stream()
		...
		assertThat(controller.contactPoints())
				.allSatisfy(point -> assertThat(point.direction()).isEqualTo(ContactDirection.INBOUND));
	}
