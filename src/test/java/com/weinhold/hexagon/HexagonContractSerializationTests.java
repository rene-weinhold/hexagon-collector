...
class HexagonContractSerializationTests {
    ...
    void serializesToContractShape() throws Exception {
        CoreInfo core = new CoreInfo("com.acme.orders.domain",
            ...
            new EventsInfo(
                List.of(new ComponentInfo("com.acme.orders.domain.event.OrderPlaced", "OrderPlaced", Provenance.ANNOTATION)),
                List.of()));
        HexagonDescriptor descriptor =
            new HexagonDescriptor(HexagonDescriptor.SCHEMA_VERSION, Instant.parse("2026-08-07T09:14:22Z"),
                new ServiceInfo("orders-service", "Orders", "3.4.1", "prod", null, "com.acme.orders", null), core,
                List.of(new PortInfo("com.acme.orders.application.port.in.PlaceOrderUseCase", "PlaceOrder", Direction.PRIMARY,
                    Provenance.ANNOTATION, List.of("placeOrder"))),
                List.of(new AdapterInfo("com.acme.orders.adapter.in.web.OrderController", "Order REST API", Direction.PRIMARY,
                    "spring-web", Provenance.ANNOTATION, List.of("com.acme.orders.application.port.in.PlaceOrderUseCase"),
        ...
        assertThat((String) ctx.read("$.ports[0].id")).isEqualTo("com.acme.orders.application.port.in.PlaceOrderUseCase");
        assertThat((String) ctx.read("$.ports[0].direction")).isEqualTo("PRIMARY");
        assertThat((String) ctx.read("$.adapters[0].implementsPorts[0]")).isEqualTo(
            "com.acme.orders.application.port.in.PlaceOrderUseCase");
        ...
        assertThat((String) ctx.read("$.adapters[0].contactPoints[0].attributes.pathTemplate")).isEqualTo("/api/orders");

        // NON_EMPTY: absent optional fields (incl. empty consumed events and unset target) must not appear.
        assertThat(json).doesNotContain("instanceId")
