...
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT,
		properties = { "spring.application.name=orders-service",
				"hexagon.collection.base-packages=com.weinhold.hexagon.web",
...
class HexagonActuatorHttpIntegrationTests {
	...
	void fetch() throws Exception {
		HttpResponse<String> response = HttpClient.newHttpClient().send(
				HttpRequest.newBuilder(URI.create("http://localhost:" + this.port + "/actuator/hexagon")).build(),
	...
	void servesTheContractOverHttp() {
		...
		assertThat((String) this.json.read("$.adapters[0].id"))
				.isEqualTo("com.weinhold.hexagon.web.OrdersApiController");
		assertThat((String) this.json.read("$.adapters[0].technology")).isEqualTo("spring-web");
		assertThat(this.json.<List<String>>read("$.adapters[0].contactPoints[*].key"))
	...
	void wildcardsRoutesThatDeclareNoHttpMethod() {
		assertThat(this.json.<List<String>>read("$.adapters[0].contactPoints[*].key"))
				.contains("http:* /api/orders/search");
		assertThat(this.json.<List<String>>read(
				"$.adapters[0].contactPoints[?(@.key == 'http:* /api/orders/search')].confidence"))
