...
class HexagonCollectionAutoConfigurationTests {
    ...
    private final ApplicationContextRunner contextRunner =
        new ApplicationContextRunner().withConfiguration(AutoConfigurations.of(HexagonCollectionAutoConfiguration.class));

    @Test
    void registersFactoryAndEndpointByDefault() {
        this.contextRunner.run(
            context -> assertThat(context).hasSingleBean(HexagonDescriptorFactory.class).hasSingleBean(HexagonEndpoint.class));
    }
    ...
    void scansConfiguredBasePackagesAndPopulatesService() {
        this.contextRunner.withPropertyValues("spring.application.name=orders-service",
            "hexagon.collection.base-packages=com.weinhold.hexagon.sample", "hexagon.collection.service.environment=prod")
                          .run(context -> {
    ...
    void skipsTheJdbcDetectorWhenThereIsNoDatabase() {
        ...
        this.contextRunner.run(context -> assertThat(context).doesNotHaveBean(JdbcDataSourceContactPointDetector.class));
    }
