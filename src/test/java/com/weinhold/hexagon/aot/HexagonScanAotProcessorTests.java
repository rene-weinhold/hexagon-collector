...
class HexagonScanAotProcessorTests {
    ...
    private final DefaultGenerationContext generationContext =
        new DefaultGenerationContext(new ClassNameGenerator(ClassName.get("com.weinhold.hexagon", "Test")), this.generatedFiles);
    ...
    void writesTheIndexAndTheHintsNeededToReadItBack() throws Exception {
        ...
        String written = this.generatedFiles.getGeneratedFileContent(Kind.RESOURCE, HexagonComponentIndex.RESOURCE_LOCATION);
        assertThat(written).contains("com.acme.Order").contains("com.acme.OrderClient");

        var hints = this.generationContext.getRuntimeHints();
        assertThat(RuntimeHintsPredicates.resource().forResource(HexagonComponentIndex.RESOURCE_LOCATION)).accepts(hints);
    ...
    void indexesEverythingEitherPassWouldClassify() {
        List<String> classNames = new HexagonScanner(List.of("com.weinhold.hexagon.conventions"), getClass().getClassLoader(),
            HexagonConventions.defaults()).relevantClassNames();
    ...
    void indexesTypesCarryingListenerMethodsSoConsumedEventsSurviveNativeCompilation() {
        List<String> classNames = new HexagonScanner(List.of("com.weinhold.hexagon.listener"), getClass().getClassLoader(),
            HexagonConventions.defaults()).relevantClassNames();
    ...
    void scansTheApplicationsAutoConfigurationPackagesByDefault() throws Exception {
        ...
        assertThat(this.generatedFiles.getGeneratedFileContent(Kind.RESOURCE, HexagonComponentIndex.RESOURCE_LOCATION)).contains(
            "com.weinhold.hexagon.conventions.adapter.in.web.OrderWebController");
    }
