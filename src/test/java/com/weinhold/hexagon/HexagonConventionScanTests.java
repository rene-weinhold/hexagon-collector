...
class HexagonConventionScanTests {
    ...
    private final HexagonScanner scanner =
        new HexagonScanner(List.of(BASE), getClass().getClassLoader(), HexagonConventions.defaults());

    @Test
    void readsPortDirectionFromThePackageLayout() {
        List<PortInfo> ports = this.scanner.scan().ports();

        assertThat(ports).extracting(PortInfo::id)
                         .containsExactly(BASE + ".application.port.in.PlaceOrderUseCase",
    ...
    void reportsOperationsInheritedFromSuperInterfaces() {
        PortInfo repository =
            this.scanner.scan().ports().stream().filter(port -> port.id().endsWith("OrderRepository")).findFirst().orElseThrow();
    ...
    void readsAdapterDirectionFromThePackageLayoutAndLinksPorts() {
        ...
        AdapterInfo controller =
            adapters.stream().filter(adapter -> adapter.id().endsWith("OrderWebController")).findFirst().orElseThrow();
        ...
        AdapterInfo persistence =
            adapters.stream().filter(adapter -> adapter.id().endsWith("OrderPersistenceAdapter")).findFirst().orElseThrow();
    ...
    void neverOverwritesAnAnnotationWithAGuess() {
        ...
        assertThat(scan.adapters()).allSatisfy(adapter -> assertThat(adapter.provenance()).isEqualTo(Provenance.ANNOTATION));
    }
